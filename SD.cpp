 ...
 *
 *  o Including `SD.h` automatically creates a global
 *    `SD` object which can be interacted with in a similar
 *    manner to other standard global objects like `Serial` and `Ethernet`.
 *
 *  o Boilerplate initialisation code is contained in one method named
 *    `begin` and no further objects need to be created in order to access
 *    the SD card.
 *
 *  o Calls to `open` can supply a full path name including parent
 *    directories which simplifies interacting with files in subdirectories.
 *
 *  o Utility methods are provided to determine whether a file exists
 ...
 *
 * o In order to handle multi-directory path traversal, functionality that
 *   requires this ability is implemented as callback functions.
 *
 * o Individual methods call the `walkPath` function which performs the actual
 ...
 *
 * o Some types of functionality will take an action at each level (e.g. exists
                          ...
                          char *buffer)
{
    ...
    // TODO: Have buffer local to this function, so we know it's the correct length?
    int bufferOffset = 0;
    int offset = *p_offset;
